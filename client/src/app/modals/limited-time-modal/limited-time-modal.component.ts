...
import { ChosePlayerNameDialogComponent } from '../chose-player-name-dialog/chose-player-name-dialog.component';
import { LimitedTimeModalComponent } from '../limited-time-modal/limited-time-modal.component';
import { WaitingRoomComponent, WaitingRoomDataPassing } from '../waiting-room/waiting-room.component';

export const LIMITED_TIME_MODE_ID = 'limitedTimeMode';

@Component({
    selector: 'app-limited-time-modal',
    templateUrl: './limited-time-modal.component.html',
    styleUrls: ['./limited-time-modal.component.css'],
})
export class LimitedTimeComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        public dialogRef: MatDialogRef<LimitedTimeModalComponent>,
    ...
    selectPlayerName(isMultiplayerGame: boolean): void {
        const dialogRef2 = this.dialog.open(ChosePlayerNameDialogComponent, { disableClose: true });
        dialogRef2.afterClosed().subscribe((isNameEntered: boolean) => {
            if (isNameEntered) {
