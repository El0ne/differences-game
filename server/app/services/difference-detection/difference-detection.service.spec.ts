...
describe('DifferenceDetectionService', () => {
    ...
    it('compareImages() should set differenceArray to the same length as firstImageArray (and secondImageArray)', async () => {
        stub(service as any, 'createArray').returns(Promise.resolve([1, 2, 3]));
        stub(service as any, 'createDifferenceImage').returns(Promise.resolve([]));
    ...
    it('compareImages() should call createArray twice and createDifferenceImage', async () => {
        const createArrayStub = stub(service as any, 'createArray').returns(Promise.resolve([1, 2, 3]));
        const createDifferenceImageStub = stub(service as any, 'createDifferenceImage').returns(Promise.resolve([]));
        await service.compareImages(TEST_IMAGE_1, TEST_IMAGE_2);
    ...
    it('createDifferenceImage should call multiple functions ', () => {
        ...
        const isPixelSameColorStub = stub(service as any, 'isPixelSameColor').returns(false);
        const setPixelBlackStub = stub(service as any, 'setPixelBlack');
        const hasWhiteNeighborStub = stub(service as any, 'hasWhiteNeighbor').returns(true);
